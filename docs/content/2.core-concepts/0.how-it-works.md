...
::steps
### 1. Initialization
At build time, the module scans your `server/auth.config.ts` and sets up the authentication system. If using NuxtHub, it also generates the database schema automatically.
...
| **Schema Generator** | Automates Drizzle table creation (with NuxtHub). |
