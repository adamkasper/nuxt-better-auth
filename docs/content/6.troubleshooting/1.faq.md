...
Yes, using [database-less mode](/guides/database-less-mode). Sessions are stored in encrypted cookies. Note that this limits some features like session revocation.
...
## "Missing auth.client.ts"

The module expects a client config file at `auth.client.ts` exporting `createAppAuthClient(baseURL)`.
...
- Confirm the file name is exactly `auth.config.ts` under your `server/` directory (or custom `serverDir`).
