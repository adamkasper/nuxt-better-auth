...
**Important**: Do not configure `secret` or `baseURL` manually - the module injects these from runtime config. For `database`, you can either:
- Use [NuxtHub Integration](/integrations/nuxthub) for automatic setup
- Configure your own database adapter following [Better Auth's database docs](https://www.better-auth.com/docs/concepts/database)
- Skip database entirely for OAuth-only mode
::
...
export default defineServerAuth(({ db, runtimeConfig }) => ({
  // db: Drizzle database instance (only with NuxtHub)
